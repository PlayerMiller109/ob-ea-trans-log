...
x = {
  ...
  BACKUP_DELETE: "删除备份",
  BACKUP_CANCEL: "取消",
  CACHE_NOT_READY: "抱歉，加载绘图文件时出错。<br><br><mark>现在有耐心，将来更省心。</mark><br><br>该插件有备份机制，但您似乎刚刚打开 Obsidian，需要等待一分钟或更长的时间来读取缓存。缓存读取完毕时，您将会在右上角收到提示。<br><br>请点击 OK 并耐心等待缓存，或者选择点击取消后手动修复文件。<br>",
  ...
  SHOW_DRAWING_OR_MD_IN_HOVER_PREVIEW_DESC:
    ...
    "当该项关闭且设置了默认打开为 Markdown 时，悬停预览将显示 Markdown 模式的绘图。<br>" +
  ...
  TRANSCLUSION_DEFAULT_WRAP_DESC:
    "除了通过 <code>![[doc#^block]]{number}</code> 中的 number 来控制折行位置，您也可以在该项 number 的默认值。<br>" +
  ...
  RENDERING_CONCURRENCY_DESC:
    "用于图像渲染的并行工作线程数。增加该值可以加快渲染速度，但可能会减慢系统的其他部分运行速度。" +
  ...
  LIVEPREVIEW_DESC:
    "开启该项，则可在 Obsidian 实时预览模式的编辑视图下，用形如 <code>![[绘图|宽度|样式]]</code> 的语法来嵌入绘图。<br>" +
    "对于已打开的文档，需要重新打开来使设置生效。" +
    "",
  FADE_OUT_EXCALIDRAW_MARKUP_NAME: "淡化 Excalidraw 标记",
  FADE_OUT_EXCALIDRAW_MARKUP_DESC: "在 Markdown 视图模式下，在 Markdown 注释 %% 之后的部分会淡化。" +
    "文本仍然存在，但视觉杂乱感会减少。请注意，您可以将 %% 放在 # Text Elements 行的上一行，" +
  ...
  TASKBONE_APIKEY_DESC: "Taskbone 的免费 API key 提供了一定数量的每月识别次数。如果您非常频繁地使用此功能，或者想要支持 Taskbone 的开发者" +
    "（您懂的，没有人能用爱发电，Taskbone 开发者也需要投入资金来维持这项 OCR 服务）您可以" +

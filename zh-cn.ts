/*
[在本地维护 Excalidraw 插件的中文翻译](https://forum-zh.obsidian.md/t/topic/50813/1)
*/
x = {
  // Sugester
  ...
  NEW_IN_NEW_PANE_EMBED:
    "新建绘图 - 于新面板 - 并以 ![[drawing]] 嵌入到当前 Markdown 文档中",
  NEW_IN_NEW_TAB_EMBED:
    "新建绘图 - 于新页签 - 并以 ![[drawing]] 嵌入到当前 Markdown 文档中",
  NEW_IN_ACTIVE_PANE_EMBED:
    "新建绘图 - 于当前面板 - 并以 ![[drawing]] 嵌入到当前 Markdown 文档中",
  NEW_IN_POPOUT_WINDOW_EMBED: "新建绘图 - 于新窗口 - 并以 ![[drawing]] 嵌入到当前 Markdown 文档中",
  TOGGLE_LOCK: "文本元素：原文模式（RAW）⟺ 预览模式（PREVIEW）",
  DELETE_FILE: "从库中删除所选图片（或以图像形式嵌入的 Markdown）源文件",
  ...
  INSERT_PDF: "以图像形式嵌入 PDF 文档到当前绘图中",
  INSERT_LAST_ACTIVE_PDF_PAGE_AS_IMAGE: "将最后激活的 PDF 页面以图像形式嵌入",
  ...
  TRAY_MODE: "面板模式 ⟺ 托盘模式（绘图工具属性页）",
  SEARCH: "搜索文本",
  CROP_PAGE: "裁剪所选页面并添加蒙版",
  CROP_IMAGE: "裁剪图片并添加蒙版",
  ANNOTATE_IMAGE: "在 Excalidraw 中标注图片",
  INSERT_ACTIVE_PDF_PAGE_AS_IMAGE: "将当前激活的 PDF 页面以图像形式嵌入",
  ...
  SEARCH_COPY_TO_CLIPBOARD_ARIA: "将整个设置对话框以 Markdown 格式复制到剪贴板。适合与 ChatGPT 等工具配合使用进行设置搜索和理解。",
  SEARCH_SHOWHIDE_ARIA: "显示/隐藏搜索栏",
  SEARCH_NEXT: "下一个",
  SEARCH_PREVIOUS: "上一个",

  //settings.ts
  NOTEBOOKLM_LINK_ARIA: "向 NotebookLM 咨询有关插件的帮助。此窗口已预加载了我所有的视频转录稿、发布说明和其他帮助内容。与 NotebookLM 聊天，探索我的 250+ 视频和 Excalidraw 文档。",
  ...
  TEMPLATE_DESC:
    ...
    "<b>模板文件：</b>比如：您的模板在默认的 Excalidraw 文件夹中且文件名是 " +
    "Template.md，则此项应设为 Excalidraw/Template.md 或 Excalidraw/Template（省略 .md 扩展名）。<br>" +
  ...
  AI_OPENAI_DEFAULT_IMAGE_MODEL_DESC:
    "使用哪个 AI 模型来生成图片（在编辑和调整图片时会强制使用 dall-e-2 模型，" +
    "因为目前只有该模型支持编辑和调整图片）。" +
    "请填写有效的 OpenAI 模型名称。" +
    "您可访问 <a href='https://platform.openai.com/docs/models'>OpenAI 网站</a> 了解更多模型信息。",
  ...
  COMPRESS_DESC:
    "Excalidraw 默认将元素记录为 JSON 格式。开启此项，可将元素的 JSON 数据以 Base64 编码" +
...
FILENAME_HEAD: "文件名",
  ...
  FILENAME_EXCALIDRAW_EXTENSION_DESC:
    "该选项在非 Excalidraw Markdown 文档（兼容模式）不会生效。<br>" +
  ...
  IFRAME_MATCH_THEME_DESC:
    ...
    "开启此项后，以交互形式嵌入到绘图中的元素将会匹配 Excalidraw 主题。<br>" +
  ...
  SHOW_DRAWING_OR_MD_IN_READING_MODE_DESC:
    ...
    "此设置不会影响：Excalidraw 模式的、嵌入到 Markdown 文档的、或悬停预览时的绘图显示。<br>" +
    "请参阅余下部分中与 <a href='#" + TAG_MDREADINGMODE + "'>Markdown Reading 模式</a> 相关的其他设置。<br>" +
    "⚠️ 关闭并重新打开 Excalidraw/Markdown 文件后生效。",
  SHOW_DRAWING_OR_MD_IN_EXPORTPDF_NAME: "在 Obsidian 中导出为 PDF 格式时 Excalidraw 将渲染为图片" ,
  ...
  GRID_DYNAMIC_COLOR_DESC:
    "<b>开启：</b>更改网格颜色以匹配画布颜色。<br><b>关闭：</b>将以下颜色用作网格颜色。",
  GRID_COLOR_NAME: "网格颜色",
  GRID_OPACITY_NAME: "网格透明度",
  GRID_OPACITY_DESC: "设置网格的不透明度。还将控制将箭头绑定到元素时绑定框的透明度。" +
    "0 全透明 ⟺ 100 不透明。",
  ...
  LINKS_HEAD_DESC: "包括：链接的打开和显示，Todo 的显示，MD-Transclusion 的显示等设置。",
  LINKS_DESC:
  ...
  FOCUS_ON_EXISTING_TAB_DESC: "当打开一个链接时，如果该文件已经打开，Excalidraw 将会聚焦到现有的标签页上 " +
    "启用此设置时，如果文件已打开，将覆盖“重用相邻窗格”，但“打开所选绘图的背景笔记”命令面板操作除外。",
  ...
  LINK_BRACKETS_DESC: `${
    ...
    "可为某个绘图单独设置，方法是在其 frontmatter 中添加如 <code>"
  ...
  EMBED_TOEXCALIDRAW_DESC: "包括：以图像形式嵌入到绘图中的 PDF 文档、以交互或图像形式嵌入到绘图中的 Markdown 文档等。",
  ...
  MD_TRANSCLUDE_WIDTH_DESC:
    "该选项会影响到折行，以及图像的宽度。<br>" +
    "可为绘图中的某个 MD-Embed 单独设置，方法是将绘图切换至 Markdown 模式，" +
    "并修改相应的 <code>[[Embed 文件名#标题|宽度x最大高度]]</code>。",
  MD_TRANSCLUDE_HEIGHT_NAME:
    "MD-Embed 的默认最大高度",
  MD_TRANSCLUDE_HEIGHT_DESC:
    "MD-Embed 的高度取决于 Markdown 文档内容的多少，但最大不会超过该值。<br>" +
    "可为绘图中的某个 MD-Embed 单独设置，方法是将绘图切换至 Markdown 模式，并修改相应的 <code>[[Embed 文件名#^块引ID|宽度x最大高度]]</code>。",
  ...
  MD_CSS_DESC:
    ...
    "可为某个 MD-Embed 单独设置，方法是在其 frontmatter 中添加如 <code>excalidraw-css: 库中的 CSS 文件或 CSS 片段</code> 的键值对。",
  EMBED_HEAD: "嵌入到 Markdown 文档中的绘图",
  EMBED_DESC: `包括：嵌入到 Markdown 文档中的绘图的预览图类型（SVG、PNG）、源文件类型（Excalidraw、SVG、PNG）、缓存、图像大小、图像主题，以及嵌入的语法等。
    此外，还有自动导出 SVG 或 PNG 文件并保持与绘图文件状态同步的设置。`,
  EMBED_CANVAS: "Obsidian 白板支持",
  EMBED_CANVAS_NAME: "沉浸式嵌入",
  EMBED_CANVAS_DESC:
    "当嵌入绘图到 Obsidian 白板中时，隐藏节点的边界和背景。" +
    "注意：如果想要背景完全透明，您依然需要设置导出背景为透明（关闭“导出图片包含背景”开关）。",
  ...
  EMBED_REUSE_EXPORTED_IMAGE_DESC:
    "该选项与下方“导出”部分“导出设置”中的 <a href='#"+TAG_AUTOEXPORT+"'>自动导出 SVG/PNG 副本</a> 选项配合使用。如果嵌入到 Markdown 文档中的绘图文件存在同名的 SVG/PNG 副本，则将其作为预览图，而不再重新生成。<br>" +
    "该选项能够提高 Markdown 文档的打开速度，尤其是当嵌入到 Markdown 文档中的绘图文件中含有大量图片或 MD-Embed 时。" +
  ...
  PREVIEW_MATCH_OBSIDIAN_DESC:
    ...
    "您可能还需要关闭“导出图片包含背景”开关，来获得与 Obsidian 更加协调的观感。",
  ...
  EMBED_TYPE_DESC:
    ...
    "也就是说，该选项不会自动帮您生成 SVG/PNG 副本，而只会引用已有的 SVG/PNG 副本。",
  ...
  EXPORT_THEME_DESC:
    "导出与绘图的黑暗/明亮主题匹配的图片。" +
    "如果关闭，在黑暗主题下导出的图片将和明亮主题一样。",
  EXPORT_EMBED_SCENE_NAME: "在导出图片中嵌入场景",
  EXPORT_EMBED_SCENE_DESC:
    "在导出图片中嵌入 Excalidraw 场景。可以在其 frontmatter 中添加如 <code>excalidraw-export-embed-scene: true/false</code> 的键值对来覆盖此设置。" +
    "此设置仅在您下次（重新）打开绘图时生效。",
  ...
  EXPORT_BOTH_DARK_AND_LIGHT_NAME: "同时导出黑暗和明亮主题风格的图片",
  ...
  DUMMY_TEXT_ELEMENT_LINT_SUPPORT_DESC: "Excalidraw 对 <code># Excalidraw Data</code> 下的文件结构非常敏感。文档的自动格式化可能会在 Excalidraw 数据中造成错误。" +
    ...
    "如果出于某些合理的原因，您决定忽略我的建议并配置了 Excalidraw 文件的自动格式化，那么可以使用这个设置。<br> " +
    "<code>## Text Elements</code> 部分对空行很敏感。一种常见的代码格式化是在章节标题后添加一个空行。但对于 Excalidraw 来说，这将破坏/改变您绘图中的第一个文本元素。" +
    "为了解决这个问题，您可以启用这个设置。启用后 Excalidraw 将在 <code>## Text Elements</code> 的开头添加一个虚拟元素，供自动格式化工具修改。" ,
  ...
  MAX_IMAGE_ZOOM_IN_DESC: "为了节省内存，并且因为 Apple Safari (Obsidian on iOS) 有一些硬编码的限制，Excalidraw.com 在放大时会限制图片和大型对象的最大分辨率。您可以使用乘数来覆盖这个限制。" +
    "这意味着将乘以 Excalidraw 默认设置的限制，乘数越大，图片放大分辨率就越高，但也会消耗更多内存。" +
    "我建议尝试多个值来设置这个参数。当您放大一个较大的 PNG 图片时，如果图片突然从视图中消失，说明已经达到了极限。默认值为 1。此设置对 iOS 无效。",
  ...
  FOURTH_FONT_DESC:
    "从您的库中选择一个 .otf/.ttf/.woff/.woff2 字体文件作为本地字体使用。" +
  ...
  CJK_ASSETS_FOLDER_DESC: `您可以在此设置 CJK 字体文件夹的位置。例如，您可以选择将其放置在 <code>Excalidraw/CJK Fonts</code> 下。<br>
    <strong>重要：</strong> 请勿将此文件夹设置为 Vault 根目录！请勿在此文件夹中放置其他字体。<br>
  ...
  SELECT_PDF: "选择要以图像形式嵌入到当前绘图中的 PDF 文档",
  ...
  // LocalFileDragAction
  LOCAL_DRAG_IMPORT: "导入文件到 Vault，或在路径来自 Vault 时复用现有文件" ,
  LOCAL_DRAG_IMAGE: "图像形式嵌入：使用本地 URI，或在路径来自 Vault 时使用内部链接" ,
  LOCAL_DRAG_LINK: "链接形式插入：使用本地 URI，或在路径来自 Vault 时使用内部链接" ,
  LOCAL_DRAG_EMBEDDABLE: "交互形式嵌入：使用本地 URI，或在路径来自 Vault 时使用内部链接" ,
